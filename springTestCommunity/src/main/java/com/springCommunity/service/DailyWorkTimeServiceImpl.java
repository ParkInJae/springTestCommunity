...
public class DailyWorkTimeServiceImpl implements DailyWorkTimeService {
	...
	/*
	  // 구글 거리계산 api private double calculateDistance(double companyLat, double companyLng, double userLat, double userLng) 
	   * { String apiKey = "YOUR_GOOGLE_API_KEY"; String apiUrl = 
	  String.format("https://maps.googleapis.com/maps/api/distancematrix/json?origins=%f,%f&destinations=%f,%f&key=%s",companyLat, companyLng, userLat, userLng, apiKey);
	  
	  RestTemplate restTemplate = new RestTemplate(); 
	  try {
	   Map<String, Object> response = restTemplate.getForObject(apiUrl, Map.class);
	   System.out.println("response=============" + response); Map<String, Object>
	   row = ((List<Map<String, Object>>) response.get("rows")).get(0);
	   ...
	   System.out.println("element =============" + element );
		   ...
		   Integer distanceInMeters =(Integer) distanceObj.get("value"); return distanceInMeters / 1000.0; // m to km 정수 / 실수 >> 실수로 결과값 나옴 
	   		} 
